...
export const roleCheck = async (req, res, next) => {
  try {
    ...
    if (!req.user || !req.user.email) {
      ...
      return res.status(401).json({ error: "Authentication required. User email not found in request." });
    }
    ...
    if (role === 'admin') {
      ...
      console.log(`Role Check Failed: User ${req.user.email} has role '${role}'. Admin privileges required.`);
      // Send a 403 Forbidden status code and a specific message
      return res.status(403).json({ error: `Access denied. Role '${role}' is not authorized to perform this action. Admin privileges required.` });
    }
