...
const OrderDetailModal = ({ showModal, setShowModal, selectedOrder, setSelectedOrder, updateOrderStatus, addTracking, handleRefund, downloadInvoice }) => (
        ...
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white p-4 rounded-lg shadow-lg max-w-md w-[90%] max-h-[70vh] overflow-y-auto">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    <div>
                        <h3 className="text-sm font-semibold text-gray-700 mb-1">Customer Information</h3>
                    ...
                    <div>
                        <h3 className="text-sm font-semibold text-gray-700 mb-1">Order Items</h3>
                        {selectedOrder.items.map((item, index) => (
                            <p key={index} className="text-xs">
                                {item.name} x {item.qty} - ${item.price.toFixed(2)}
                            </p>
                        ))}
                        <p className="font-semibold mt-1 text-sm">Total: ${selectedOrder.amount.toFixed(2)}</p>
                    </div>
                </div>
                <div className="mt-3">
                    <h3 className="text-sm font-semibold text-gray-700 mb-1">Order Status</h3>
                        ...
                        {selectedOrder.status === 'Shipped' && (
                            <div className="flex space-x-2">
                                ...
                                <button
                                    ...
                                    onClick={() => {
                                        const carrier = document.getElementById(`carrier-${selectedOrder.id}`).value;
                                        const trackingNumber = document.getElementById(`tracking-${selectedOrder.id}`).value;
                                        addTracking(selectedOrder.id, carrier, trackingNumber);
                                    }}
                                >
                                    Update Tracking
                                </button>
                ...
                {selectedOrder.refundStatus && (
                    <div className="mt-3">
                        <h3 className="text-sm font-semibold text-gray-700 mb-1">Return/Refund Management</h3>
                        <p className="text-sm"><strong>Status:</strong> {selectedOrder.refundStatus}</p>
