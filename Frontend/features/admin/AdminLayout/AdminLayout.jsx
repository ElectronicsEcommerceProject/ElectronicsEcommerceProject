...
const AdminLayout = () => {
  ...
  const menuItems = [
    ...
    },
    {
      ...
      section: "orders",
      icon: <ShoppingCartSimple size={24} weight="bold" className="text-red-500" />,
    },
    {
  ...
  return (
    <div className="bg-gray-100 font-sans min-h-screen flex flex-col">
      ...
      <div className="flex flex-1 pt-2">
        ...
        {/* Sidebar (Drawer) */}
        <aside
          className={`w-64 bg-white shadow-md p-4 fixed left-0 h-[calc(100vh-4.5rem)] z-40 transition-transform duration-300 ease-in-out overflow-y-auto top-[4.5rem] ${sidebarOpen ? "translate-x-0" : "-translate-x-full"} md:translate-x-0`}
        >
          <nav className="space-y-2">
            <div>
              ...
              <div
                className={`space-y-2 overflow-hidden transition-all duration-300 ${accordionOpen ? "max-h-screen" : "max-h-0"}`}
              >
                {menuItems.map((item) => (
                  <div
                    key={item.section}
