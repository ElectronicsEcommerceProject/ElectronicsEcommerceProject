...
import {
  ...
  FaShoppingCart, FaTimes, FaSearch, FaFilter, FaTimes as FaClear, FaChevronDown,
...
const StockManagement = () => {
  ...
  const filteredVariants = useMemo(() => {
    ...
    switch (activeFilter) {
      ...
      case 'in_carts_wishlists': filtered = filtered.filter(v => (v.in_carts_wishlists || 0) > 0); break;
  ...
  const handleExportCSV = () => {
    const headers = ['Variant ID', 'Product Name', 'Brand', 'Category', 'Variant Name', 'Price', 'Stock Quantity', 'In Carts/Wishlists', 'Available', 'Sold', 'Status', 'Last Updated'];
      ...
      <div className="bg-white p-12 rounded-2xl shadow-2xl border border-gray-100 max-w-md w-full">
        <div className="flex flex-col items-center space-y-6">
          ...
          <div className="text-center">
            <h2 className="text-2xl font-bold text-gray-800 mb-2">Loading Stock Data</h2>
            <p className="text-gray-600">Please wait while we fetch the latest inventory information...</p>
          </div>
    ...
    <div className="bg-gradient-to-br from-gray-50 to-gray-100 min-h-screen p-2 md:p-4 lg:p-6 max-w-full overflow-x-hidden">
      ...
      <div className="bg-white p-3 md:p-4 lg:p-6 rounded-lg shadow-md mb-4 border border-gray-100">
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
          <div className="flex-1">
            <h1 className="text-xl lg:text-2xl font-bold text-gray-800 mb-2">Stock Management</h1>
      ...
      <div className="bg-white p-3 md:p-4 lg:p-6 rounded-lg shadow-md mb-4 border border-gray-100">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3">
          <SearchableDropdown options={categories} value={categoryFilter} onChange={setCategoryFilter} placeholder="Category" valueKey="category_id" />
          <SearchableDropdown options={brands} value={brandFilter} onChange={setBrandFilter} placeholder="Brand" valueKey="brand_id" />
          <SearchableDropdown options={uniqueProducts} value={productFilter} onChange={setProductFilter} placeholder="Product" />
          <SearchableDropdown options={uniqueVariants} value={variantFilter} onChange={setVariantFilter} placeholder="Variant" displayKey="name" />
          <SearchableDropdown options={[{ id: 'In Stock', name: 'In Stock' }, { id: 'Low', name: 'Low' }, { id: 'Out of Stock', name: 'Out of Stock' }]} value={statusFilter} onChange={setStatusFilter} placeholder="Status" />
        </div>

        {getActiveFilters().length > 0 && (
          <div className="mt-4 p-3 bg-blue-50 rounded-lg border border-blue-200">
            <span className="text-sm font-medium text-blue-800 mb-2 block">Active Filters ({getActiveFilters().length}):</span>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-3 mb-6">
        {[
          ...
          <div
            key={card.key}
            className={`p-3 rounded-lg shadow cursor-pointer transition-all duration-200 ${activeFilter === card.key ? `bg-${card.color}-100 border-${card.color}-500 border-2` : 'bg-white hover:shadow-md'}`}
            onClick={() => setActiveFilter(card.key)}
            title={card.tooltip || ''}
          >
            <div className="flex flex-col items-center text-center space-y-1">
              <card.icon className={`text-${card.color}-500 text-lg flex-shrink-0`} />
              <div className="min-w-0">
                <h3 className="font-semibold text-gray-700 text-xs leading-tight truncate">{card.title}</h3>
      ...
      <div className="bg-white p-3 md:p-4 lg:p-6 rounded-lg shadow-md border border-gray-100">
        ...
        <div className="block md:hidden space-y-4">
          {paginatedVariants.map(variant => (
            <div key={variant.product_variant_id} className="border rounded-lg p-4 bg-gray-50">
              <div className="flex justify-between items-start mb-3">
                <div className="flex-1 min-w-0">
                  <h3 className="font-semibold text-gray-800 truncate">{variant.product_name}</h3>
                  <p className="text-sm text-gray-600 truncate">{variant.variant_name}</p>
                  <p className="text-xs text-gray-500">{variant.brand_name} • {variant.category_name}</p>
                </div>
                <div className="flex space-x-1 ml-2">
                  <button
                    title="Update Stock"
                    className="bg-blue-600 text-white p-2 rounded hover:bg-blue-700 text-xs"
                    onClick={() => { setCurrentVariant(variant); setNewStockQuantity(variant.stock_quantity || 0); setShowStockModal(true); }}
                  >
                    <FaBoxes />
                  </button>
                  <button
                    ...
                    onClick={() => { setCurrentVariant(variant); setShowDetailsModal(true); }}
                  >
                    <FaInfoCircle />
                  </button>
                </div>
              ...
              <div className="grid grid-cols-2 gap-3 text-sm">
                <div className="flex justify-between">
                  ...
                  <span className="font-semibold">₹{(variant.price || 0).toFixed(2)}</span>
                </div>
                <div className="flex justify-between">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-semibold text-orange-600">{variant.in_carts_wishlists || 0}</span>
                </div>
                <div className="flex justify-between">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-semibold text-purple-600">{variant.sold || 0}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-600">Status:</span>
                  <span className={`font-semibold ${variant.status === 'In Stock' ? 'text-green-600' : variant.status === 'Low' ? 'text-yellow-600' : 'text-red-600'}`}>
                    {variant.status}
                  </span>
                </div>
              </div>
            ...
            <table className="w-full border-collapse bg-white" style={{minWidth: '800px'}}>
              <thead>
                <tr className="bg-gradient-to-r from-gray-50 to-gray-100 border-b border-gray-200">
                  <th className="px-2 py-2 text-left text-gray-700 font-semibold text-xs uppercase tracking-wide w-[15%]">
                    <div className="flex items-center space-x-1 cursor-pointer" onClick={() => requestSort('product_name')}>
                      <span>Product</span>
                      {sortConfig.key === 'product_name' && <span className="text-blue-600">{sortConfig.direction === 'asc' ? '↑' : '↓'}</span>}
                    </div>
                  </th>
                  <th className="px-2 py-2 text-left text-gray-700 font-semibold text-xs uppercase tracking-wide w-[10%]">
                    <div className="flex items-center space-x-1 cursor-pointer" onClick={() => requestSort('brand_name')}>
                      <span>Brand</span>
                      {sortConfig.key === 'brand_name' && <span className="text-blue-600">{sortConfig.direction === 'asc' ? '↑' : '↓'}</span>}
                    </div>
                  </th>
                  <th className="px-2 py-2 text-left text-gray-700 font-semibold text-xs uppercase tracking-wide w-[12%]">
                    <div className="flex items-center space-x-1 cursor-pointer" onClick={() => requestSort('variant_name')}>
                      <span>Variant</span>
                      {sortConfig.key === 'variant_name' && <span className="text-blue-600">{sortConfig.direction === 'asc' ? '↑' : '↓'}</span>}
                    </div>
                  </th>
                  <th className="px-2 py-2 text-left text-gray-700 font-semibold text-xs uppercase tracking-wide w-[8%]">
                    <div className="flex items-center space-x-1 cursor-pointer" onClick={() => requestSort('price')}>
                      <span>Price</span>
                      {sortConfig.key === 'price' && <span className="text-blue-600">{sortConfig.direction === 'asc' ? '↑' : '↓'}</span>}
                    </div>
                  </th>
                  <th className="px-2 py-2 text-left text-gray-700 font-semibold text-xs uppercase tracking-wide w-[6%]">
                    <div className="flex items-center space-x-1 cursor-pointer" onClick={() => requestSort('stock_quantity')}>
                      <span>Stock</span>
                      {sortConfig.key === 'stock_quantity' && <span className="text-blue-600">{sortConfig.direction === 'asc' ? '↑' : '↓'}</span>}
                    </div>
                  </th>
                  <th className="px-2 py-2 text-left text-gray-700 font-semibold text-xs uppercase tracking-wide w-[6%]">
                    <div className="flex items-center space-x-1 cursor-pointer" onClick={() => requestSort('in_carts_wishlists')}>
                      <span>Carts</span>
                      {sortConfig.key === 'in_carts_wishlists' && <span className="text-blue-600">{sortConfig.direction === 'asc' ? '↑' : '↓'}</span>}
                    </div>
                  </th>
                  <th className="px-2 py-2 text-left text-gray-700 font-semibold text-xs uppercase tracking-wide w-[7%]">
                    <div className="flex items-center space-x-1 cursor-pointer" onClick={() => requestSort('availableStock')}>
                      <span>Available</span>
                      {sortConfig.key === 'availableStock' && <span className="text-blue-600">{sortConfig.direction === 'asc' ? '↑' : '↓'}</span>}
                    </div>
                  </th>
                  <th className="px-2 py-2 text-left text-gray-700 font-semibold text-xs uppercase tracking-wide w-[6%]">
                    <div className="flex items-center space-x-1 cursor-pointer" onClick={() => requestSort('sold')}>
                      <span>Sold</span>
                      {sortConfig.key === 'sold' && <span className="text-blue-600">{sortConfig.direction === 'asc' ? '↑' : '↓'}</span>}
                    </div>
                  </th>
                  <th className="px-2 py-2 text-left text-gray-700 font-semibold text-xs uppercase tracking-wide w-[8%]">
                    <div className="flex items-center space-x-1 cursor-pointer" onClick={() => requestSort('status')}>
                      <span>Status</span>
                      {sortConfig.key === 'status' && <span className="text-blue-600">{sortConfig.direction === 'asc' ? '↑' : '↓'}</span>}
                    </div>
                  </th>
                  <th className="px-2 py-2 text-left text-gray-700 font-semibold text-xs uppercase tracking-wide w-[12%]">
                    Actions
              ...
              <tbody className="divide-y divide-gray-200">
                {paginatedVariants.map((variant, index) => (
                  ...
                  >
                    <td className="px-2 py-2 text-gray-900 font-medium text-xs" title={variant.product_name}>
                      <div className="truncate">{variant.product_name}</div>
                    </td>
                    <td className="px-2 py-2 text-gray-700 text-xs" title={variant.brand_name}>
                      <div className="truncate">{variant.brand_name}</div>
                    </td>
                    <td className="px-2 py-2 text-gray-700 text-xs" title={variant.variant_name}>
                    ...
                    <td className="px-2 py-2">
                      <span className={`inline-flex px-1 py-0.5 rounded text-xs font-semibold ${
                        ...
                        {variant.status === 'In Stock' ? 'In' : variant.status === 'Out of Stock' ? 'Out' : variant.status}
                      </span>
                    </td>
                    <td className="px-2 py-2">
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50 p-4">
          <div className="bg-white p-4 md:p-6 rounded-lg w-full max-w-lg max-h-screen overflow-y-auto">
            <h2 className="text-lg md:text-xl font-bold mb-4">
              Update Stock: <span className="text-blue-600">{currentVariant.variant_name}</span>
            </h2>
            <div className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium mb-1">Variant ID</label>
                  <input className="w-full p-2 border rounded-lg bg-gray-100 text-sm" value={currentVariant.product_variant_id} readOnly />
                </div>
                <div>
                  <label className="block text-sm font-medium mb-1">Variant Name</label>
                  <input className="w-full p-2 border rounded-lg bg-gray-100 text-sm" value={currentVariant.variant_name} readOnly />
                </div>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium mb-1">Brand</label>
                  <input className="w-full p-2 border rounded-lg bg-gray-100 text-sm" value={currentVariant.brand_name} readOnly />
                </div>
                <div>
                  <label className="block text-sm font-medium mb-1">Category</label>
                  <input className="w-full p-2 border rounded-lg bg-gray-100 text-sm" value={currentVariant.category_name} readOnly />
                </div>
              </div>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
                <div>
                  <label className="block text-xs md:text-sm font-medium mb-1">Stock</label>
                  <input className="w-full p-2 border rounded-lg bg-gray-100 text-blue-600 text-sm" value={currentVariant.stock_quantity || 0} readOnly />
                </div>
                <div>
                  <label className="block text-xs md:text-sm font-medium mb-1">In Carts</label>
                  <input className="w-full p-2 border rounded-lg bg-gray-100 text-orange-600 text-sm" value={currentVariant.in_carts_wishlists || 0} readOnly />
                </div>
                <div>
                  <label className="block text-xs md:text-sm font-medium mb-1">Available</label>
                  <input className="w-full p-2 border rounded-lg bg-gray-100 text-green-600 text-sm" value={currentVariant.availableStock} readOnly />
                </div>
                <div>
                  <label className="block text-xs md:text-sm font-medium mb-1">Sold</label>
                  <input className="w-full p-2 border rounded-lg bg-gray-100 text-purple-600 text-sm" value={currentVariant.sold || 0} readOnly />
                </div>
              </div>
              <div className="border-t pt-4">
                <h3 className="text-base md:text-lg font-semibold mb-3">Update Stock Quantity</h3>
                <div>
                  ...
                  <div className="mt-4 p-3 bg-blue-50 rounded-lg">
                    <div className="grid grid-cols-2 gap-2 text-sm">
                      ...
                      <p><strong>Change:</strong>
                        <span className={newStockQuantity - (currentVariant.stock_quantity || 0) >= 0 ? 'text-green-600' : 'text-red-600'}>
                          {newStockQuantity - (currentVariant.stock_quantity || 0) >= 0 ? '+' : ''}{newStockQuantity - (currentVariant.stock_quantity || 0)}
                        </span>
                      </p>
                      <p><strong>New Available:</strong>
                        <span className="text-green-600">{Math.max(0, newStockQuantity - (currentVariant.in_carts_wishlists || 0))}</span>
                      </p>
                    </div>
                  </div>
                )}
              ...
              <div className="flex flex-col sm:flex-row sm:justify-end gap-2 pt-4 border-t">
                <button
                  ...
                  onClick={() => { setShowStockModal(false); setNewStockQuantity(0); }}
                >
      ...
      {showEditModal && currentVariant && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50 p-4">
          <div className="bg-white p-4 md:p-6 rounded-lg w-full max-w-lg max-h-screen overflow-y-auto">
            <h2 className="text-lg md:text-xl font-bold mb-4">
              Edit Variant: <span className="text-green-600">{currentVariant.variant_name}</span>
            </h2>
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium mb-1">Variant Name</label>
              ...
              <div className="flex flex-col sm:flex-row sm:justify-end gap-2 pt-4 border-t">
                <button
                  className="bg-gray-500 text-white px-4 py-2 rounded-lg hover:bg-gray-600 flex items-center justify-center"
                  onClick={() => { setShowEditModal(false); setEditVariantName(''); setEditPrice(0); }}
                >
                  <FaTimes className="mr-2" /> Cancel
                </button>
                <button
      ...
      {showDetailsModal && currentVariant && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50 p-4">
          <div className="bg-white p-4 md:p-6 rounded-lg w-full max-w-lg max-h-screen overflow-y-auto">
            <h2 className="text-lg md:text-xl font-bold mb-4">
              View Details: <span className="text-gray-600">{currentVariant.variant_name}</span>
            </h2>
            <div className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium mb-1">Variant ID</label>
                  <p className="p-2 text-sm bg-gray-100 rounded-lg break-all">{currentVariant.product_variant_id}</p>
                </div>
                <div>
                  <label className="block text-sm font-medium mb-1">Variant Name</label>
                  <p className="p-2 text-sm bg-gray-100 rounded-lg">{currentVariant.variant_name}</p>
                </div>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium mb-1">Product</label>
                  <p className="p-2 text-sm bg-gray-100 rounded-lg">{currentVariant.product_name}</p>
                </div>
                <div>
                  <label className="block text-sm font-medium mb-1">Brand</label>
                  <p className="p-2 text-sm bg-gray-100 rounded-lg">{currentVariant.brand_name}</p>
                </div>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium mb-1">Category</label>
                  <p className="p-2 text-sm bg-gray-100 rounded-lg">{currentVariant.category_name}</p>
                </div>
                <div>
                  <label className="block text-sm font-medium mb-1">Price</label>
                  <p className="p-2 text-sm bg-gray-100 rounded-lg font-semibold">₹{currentVariant.price.toFixed(2)}</p>
                </div>
              </div>
              ...
              <div className="bg-blue-50 p-4 rounded-lg">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="text-center">
                    <label className="block text-xs font-medium text-gray-600">Current Stock</label>
                    <p className="text-lg font-bold text-blue-600">{currentVariant.stock_quantity}</p>
                  </div>
                  <div className="text-center">
                    <label className="block text-xs font-medium text-gray-600">In Carts/Wishlists</label>
                    <p className="text-lg font-bold text-orange-600">{currentVariant.in_carts_wishlists || 0}</p>
                  </div>
                  <div className="text-center">
                    <label className="block text-xs font-medium text-gray-600">Available</label>
                    <p className="text-lg font-bold text-green-600">{currentVariant.availableStock}</p>
                  </div>
                  <div className="text-center">
                    <label className="block text-xs font-medium text-gray-600">Sold</label>
                    <p className="text-lg font-bold text-purple-600">{currentVariant.sold}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="flex justify-end pt-4 border-t">
                <button
                  ...
                  onClick={() => { setShowDetailsModal(false); setCurrentVariant(null); }}
                >
